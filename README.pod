...
If a subdirectory C<CORPUS_ROOT/config.local/> exists, it will be treated as a (sparse) local set 
of dstar-configuration overrides for L<C<CORPUS>|/-c CORPUS> and merged into a pure-local
configuration directorsy C<CORPUS_ROOT/config/> by implicit calls to  C<dstar-checkout-corpus.sh>.
...
to "production" hosts by means of the L</publish> action is B<not> sufficient to allow integration
of new dstar functionality or to enable many bug-fixes.  You are free to delete or archive corpus build
...
If you are using a C<CORPUS_ROOT/config.local/> directory for corpus configuration
