...
pulled from the ZDL docker registry at C<https://docker.zdl.org>.
...
need an SSH identity for resource-synchronization via C<rsync> (which is
probably broken anyways).
...
Default is C<docker.zdl.org/dstar/dstar-buildhost:latest>.
If you don't have either a local image or ZDL docker registry credentials,
you should probably set this to C<cudmuncher/dstar-buildhost:latest>.
...
It is possible to use C<dstar-gantry.sh> together with a specialized
L<C<dstar-rcbuildhost>|https://git.zdl.org/svn-mirror/dstar-docker/src/branch/master/build/dstar-rcbuildhost>
...
L<hub.docker.com/u/cudmuncher|https://hub.docker.com/u/cudmuncher> pre-built docker images
