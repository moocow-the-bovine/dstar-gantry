...
D* corpus operations using the latest public C<dstar-buildhost> docker image
from L<cudmuncher/dstar-buildhost|https://hub.docker.com/r/cudmuncher/dstar-buildhost>
(formerly pulled from the private ZDL docker registry at C<https://docker.zdl.org>).
...
C<dstar-gantry.sh> requires the debian packages: C<bash>, C<openssh-client>, and C<subversion>
...
See L<"https://docs.docker.com/get-docker/"|https://docs.docker.com/get-docker/> for
...
L<C<dstar-buildhost>|https://hub.docker.com/r/cudmuncher/dstar-buildhost>
image from docker hub.

If you are using the private C<docker.zdl.org> registry and
wish to make use of gantry's C<pull> action to acquire the
...
Due to updates of the underlying ZDL docker repository, the default repository
URL has changed from C<lex.dwds.de:443> to C<docker.zdl.org> in C<dstar-gantry.sh> v0.0.4 (2021-04-21),
which will require users to (re-)execute a C<docker login> for the new
repository, C<docker.zdl.org>.
...
If you have no ZDL docker credentials, you can use the public docker images (e.g. C<cudmuncher/dstar-buildhost:latest>)
available on dockerhub, but be aware that the dockerhub images may not be up-to-date with those from the ZDL registry.

=item ssh-agent

You can skip this section if you only need to access public resources.
As of 2025-06-09, C<gantry_dstar_svnroot> defaults to the (read-only) public
repo C<https://svn.code.sf.net/p/ddc-dstar-core/code/ddc-dstar/trunk>,
so you you shouldn't need SSH for SVN checkouts anymore.  You may still
need an SSH identity for resource-synchronization via C<rsync> (which you
shouldn't need unless you need to sync non-free resources).  It's always
a good idea to run an C<ssh-agent> though, so feel free to read on if you
...
C<$gantry_dstar_svnroot> (formerly C<svn.dwds.de>), as whatever user your
L<C<~/.ssh/config>|https://linux.die.net/man/5/ssh_config>
...
authorized for password-free access to C<ddc@svn.dwds.de>.

If you wish to make use of gantry's
automatic non-free resource synchronization features (default, recommended),
...
If you only need to access free resources, you can stick with the default
synchronization over HTTPS (via C<CABRC_FREE=yes>).
...
Checkout the gantry project itself from git or SVN to your local machine,
...
If you have DWDS SVN credentials, you can also use the original SVN repository
(which is no longer maintained by the original author):
...
Download the latest C<dstar-buildhost> image from the default docker registry
with the L<C<pull>|/pull> action:

If you don't have ZDL docker registry credentials, use the public images from dockerhub
(now the default):
...
You can ignore any warnings about failed C<PUBLISH> tests ... unless you
want to use gantry to publish corpus builds using rsync+ssh, in which case you should
set your configuration variables accordingly and ensure that your C<ssh-agent> is serving
up the right keys.
...
non-interactively (C<-batch>), or detached in the background (C<-bg>).
Foreground (C<-fg>) operation is equivalent to the
C<docker-run> C<--tty=true> and  C<--interactive=true> options,
batch (C<-batch>) operation is equivalent to the
C<docker-run> C<--tty=false> and  C<--interactive=false> options,
...
Specifying this option will allow C<dstar-gantry.sh> to invoke a subordinate
C<dstar-buildhost> container even if it does not detect a running
L<C<ssh-agent>|/SSH_AUTH_SOCK>, and will bypass any running agent by
unsetting the L<C<SSH_AUTH_SOCK>|/SSH_AUTH_SOCK> environment variable
for the host C<dstar-gantry.sh> process itself.  This can be useful if you're
performing only container-local operations which don't require external authentication
(e.g. L<C<run>|/run> with L<C<dstar_checkout_corpus_opts=-dummy>|/dstar_checkout_corpus_opts>).
Without this option, C<dstar-gantry.sh> will issue an error message and exit
with abnormal status unless it detects a running C<ssh-agent> with at least
one registered identity, which is usually The Right Thing To Do (TM).
...
See the example
L<C<dstar-gantry.rc> file|https://git.zdl.org/svn-mirror/dstar-gantry/src/branch/master/dstar-gantry.rc> in the C<dstar-gantry> distribution for a list of available variables.
...
Default is C<cudmuncher/dstar-buildhost:latest>
(formerly C<docker.zdl.org/dstar/dstar-buildhost:latest>).
...
B<BROKEN 2025-06-22>: the default CAB server won't run without non-free resources.
...
Outside of the DWDS/ZDL ecosystem, it's usually a good idea to set C<-u $(id -un)>.
...
Outside of the DWDS/ZDL ecosystem, it's probably a good idea to set C<-g $(id -gn)>.
...
Attempts to synchronize the local C<dstar-gantry> checkout via C<git pull> or C<svn update>.
If the C<dstar-gantry.sh> script (or symlink) does not resolve to a git or SVN working copy
...
the selected docker registry (docker hub public or ZDL private).
May require a preceding L<C<docker login>|/registry credentials>.
...
options (e.g. to bind container ports other than 80), this is how to do it.
...
L<incremental corpus updates|$doc/README_build.html#Incremental-Update>
...
As of 2025-06-22, you can use the (slightly slimmer)
L<C<dstar-webhost>|https://hub.docker.com/r/cudmuncher/dstar-webhost> image
if you only need the C<run> command.
You can probably get away with the
L<C<dstar-runhost>|https://hub.docker.com/r/cudmuncher/dstar-runhost> image
if you need only DDC and CAB server(s) (i.e. no HTTP UI or REST API).

=item freeze

Archives the C<L<CORPUS_ROOT|/-C CORPUS_ROOT>/build/> subdirectory to
C<L<CORPUS_ROOT|/-C CORPUS_ROOT>/build-freeze.tar.gz> and removes the
original C<build/> subdirectory on success.  Can be used together with the C<L<thaw|/thaw>> action
to reduce long-term storage space
requirements for infrequently updated corpora.

=item thaw

Unpacks C<L<CORPUS_ROOT|/-C CORPUS_ROOT>/build-freeze.tar.gz> archive(s)
as created by the C<L<freeze|/freeze>> action to
C<L<CORPUS_ROOT|/-C CORPUS_ROOT>/build/> and removes the original
C<build-freeze.tar.gz> archive on success.
If you C<L<freeze|/freeze>> your C<build/> subdirectory, it will
need to be C<L<thaw|/thaw>>ed before you can perform any
build-time container operations (e.g.
C<build>,
C<update>,
C<update-meta>,
C<test>,
...
If set to non-empty and non-"C<no>", any attempt to sync CAB resources will use
the old C<rsync+ssh> strategy for syncing non-free resources.  As of
2025-06-21, the default C<yes> will attempt to sync only publicly available
CAB resources via HTTPS using C<lftp>.
...
See the L<C<-no-agent>|/-a, -no-agent> if you really need to run
...
=item dstar_build_timezone

Timezone to use within the container. If unset (default), the default container
timezone will be used (usually C<UTC>).  This is mostly useful for
formatting timestamps in log-files produced by L<C<cronit.perl>|$doc/HOWTO.html#cronit.perl>.
A useful idiom to set the container timezone to that of the (debian) gantry host
is:
...
See
L<C<dstar-buildhost:/dstar/init/etc_default_dstar_relay>|https://git.zdl.org/svn-mirror/ddc-dstar/src/branch/master/init/etc_default_dstar_relay> for syntax and more details.


=item dstar_selftest_publish

Set to non-empty and non-"C<no>" to make the C<self-test> action test SSH access
for C<publish> operations.  Disabled by default.
...
Outside of the DWDS/ZDL ecosystem, you might want to set up a corpus checkout
using a self-contained `config` subdirectory
without any SVN access at all
by manually copying the C</dstar/corpus.template> directory from the C<dstar-buildhost> image:
...
gantry L<C<-bg>|/-fg, -batch, -bg> or L<C<docker run -d>|https://docs.docker.com/engine/reference/run/#detached--d> option
...
=head2 Example: Partial Corpus Build

Before attempting to (partially) build a corpus, you should ensure that
you have fulfilled all the L</Common Prerequisites>.

"Partial" corpus builds are simply gantry corpus L<C<build>|/build> operations
which invoke only a subset of the full
L<annotation and indexing|$doc/HOWTO.html#Annotate-and-Build>
procedure supported by the
dstar build system, typically by setting
the L<container environment variable|/-e VAR=VALUE>
C<STAGES>
to specify the desired L<build stages|$doc/README_build.html#Stage-Wise-Builds>.

=over 4

=item MYCORPUS partial build: tokenization

 $ dstar-gantry.sh -bg -c MYCORPUS -eSTAGES=1 build

Executing a partial C<stage1> build
performs
TEI-XML source checking (L<C<build/src_catalog/>|$doc/README_build.html#src_catalog>),
and XML header extraction (L<C<build/xml_header/>|$doc/README_build.html#xml_header>),
serialization and tokenization (L<C<build/xml_tok/>|$doc/README_build.html#xml_tok>)
by default.

=item MYCORPUS partial build: annotation

 $ dstar-gantry.sh -bg -c MYCORPUS -eSTAGES=2 build

Executing a partial C<stage2> build
requires prior successful completion of a L<C<stage1> build|/MYCORPUS partial build: tokenization>,
and performs
corpus analysis in (L<C<build/cab_corpus/>|$doc/README_build.html#cab_corpus>).

=item MYCORPUS partial build: tokenization and annotation

 $ dstar-gantry.sh -bg -c MYCORPUS -eSTAGES="1 2" build

You can combine multiple stages in a single partial build:
the above example performs a
L<C<stage1> build|/MYCORPUS partial build: tokenization>
followed by a
L<C<stage2> build|/MYCORPUS partial build: annotation>
in a single call.

=item MYCORPUS partial build: pre-indexing

 $ dstar-gantry.sh -bg -c MYCORPUS -eSTAGES="1 2 3" STAGE3_DIRS="ddc_xml" build

Partial builds are not restricted to whole stages:
you can select specific C<MYCORPUS/build/> subdirectories from a stage
C<I<i>> by setting the C<STAGEI<i>_DIRS> variable.
The above example performs all corpus build operations up to but not
including configuration and compilation of the DDC index
(i.e. full C<stage1> and C<stage2> builds and conversion of annotated documents
to the legacy L<C<ddc_xml>|$doc/README_build.html#ddc_xml> format suitable for input to a C<ddc_index> process).

=item MYCORPUS partial build: incremental builds
...
sources to an existing partial build and re-invoking the
partial build action should cause only those source documents which have
been added or changed to be (re-)analyzed.

Note that changes
in any shared resources in L<C<RESOURCES_DIR>|/-R RESOURCES_DIR>
may also cause I<all> documents to be re-analyzed on a second
build invocation: if you need to ensure stable resources for
incremental (partial) builds, use a dedicated L<C<RESOURCES_DIR>|/-R RESOURCES_DIR>
and specify the gantry L<C<-RO>|/-RO> option for all but the initial
L<build|/build> invocation.

Note also that for incremental (partial) builds,
a certain amount of computational overhead is required by GNU make to
determine I<which> sources are new rsp. whether any previously registered
sources have been updated, which may be a problem for very large corpora with
many small documents.  If you need perform incremental builds on such a large
corpus, you will probably need to optimize the frequency of your
L<build|/build> calls to avoid wasting resources on unnecessary overhead,
...
incremental partial gantry builds)
and restricting your updates to a single block at a time (e.g. the
smallest),
or explicitly setting the L<C<XML_SRC_DIRS>|$doc/README_build.html#XML_SRC_DIRS>
and/or L<C<XML_SRC_FIND>|$doc/README_build.html#XML_SRC_FIND> make variables
to select only a particular subset (e.g. newly added files) of the available sources.

=item MYCORPUS partial build: re-indexing
...
ensure that all C<make> prerequisites of the partial build you're executing
actually exist.  See
L<C<HOWTO: Dataflow>|$doc/README_build.html#Dataflow> for an approximate
overview of the dependencies between the various build directories,
and see the individual C<Makefile>s for a formal specification of
the file-level dependencies.  When in doubt, try it out: if C<make>
complains about missing prerequisites, you probably need to figure
out a way to create or simulate them.

If -- as in this example -- you're just interested in (re-)creating
corpus index data after successfully completing at least C<stage1> and C<stage2>
and then manually modifying (but not adding or deleting)
intermediate files under (for example)
L<C<xml_header/>|$doc/README_build.html#xml_header>,
L<C<cab_corpus/>|$doc/README_build.html#cab_corpus>,
or even
L<C<ddc_xml/>|$doc/README_build.html#ddc_xml>,
you should be able to safely re-engage the build process
starting from L<C<stage3>|$doc/README_build.html#Stage-Wise-Builds>.

If you haven't actually populated the C<stage1> and C<stage2> build
subdirectories and wish to begin the indexing process from C<stage3>,
your best option is probably a
L<legacy DDC-XML import|$doc/README_build.html#Importing-legacy-DDC-XML-files>.
...
=head2 Example: Building Language Resources

It is possible to use C<dstar-gantry.sh> together with a specialized (non-free)
L<C<dstar-rcbuildhost>|https://github.com/moocow-the-bovine/dstar-docker/tree/main/build/dstar-rcbuildhost>
(ZDL: L<C<dstar-rcbuildhost>|https://git.zdl.org/svn-mirror/dstar-docker/src/branch/master/build/dstar-rcbuildhost>)
docker image (via the L<C<-i> option|/-i IMAGE>)
and appropriate L<sources|https://git.zdl.org/svn-mirror/dstar-resources-build/src/branch/master/src>
to L<compile CAB language resources|https://git.zdl.org/svn-mirror/dstar-resources-build/src/branch/master/README.txt>
to a host directory suitable for use with the L<C<-R>|/-R RESOURCES_DIR>
option.

=over 4

=item Retrieve the C<dstar-rcbuildhost> image
...
=item Checkout resource-build output directory

 HOST$ svn checkout svn+ssh://svn.dwds.de/home/svn/dev/ddc-dstar/trunk/resources resources

This directory will contain the built resources; it should also contain some configuration
files which live in SVN, so it's easiest to checkout the SVN project first and build into that.

=item Setup non-SVN resource-build sources

 HOST$ rsync -vau kira.bbaw.de:/home/ddc-dstar/arc/resources/src/ resources-src

See L<C<dstar-resources-build/src/README.md>|https://git.zdl.org/svn-mirror/dstar-resources-build/src/branch/master/src/README.md>
for more details.

=item Build default resources
...
=item Build specific resources

Alternatively, you can choose to (re-)build resources for only a single "language", e.g.
C<en_wsj>, C<de_dta>, or C<de_dstar> by calling
...
On completion, the resources you have (re-)built should be available under your
L<C<resources/> checkout|/Checkout resource-build output directory>.
...
=head2 Example: Persistent Language Resource Builds

In L<Example: Building Language Resources>, intermediate resource build files
are not saved.  This can be irritating and time-consuming if you need to do it
often.  In that case, it's probably best to export the whole
C</dstar/resources> subtree from the container and just bind-mount that in.

=over 4

=item Preliminaries

See L<Retrieve the C<dstar-rcbuildhost> image>.

=item Export C</dstar/resources> subtree from the image

 HOST$ docker run --rm dstar-rcbuildhost tar cC /dstar resources | tar x

This should create a C<resources/> directory on the host with a C<resources/build>
subdirectory suitable for use by the C<dstar-rcbuildhost> image.

=item Inject non-SVN resource-build sources
...
=item Build default resources and intermediate data
...
Analogous to L<Build default resources>, but leaves intermediate build
files in place under C<resources/build>, which can be re-used by subsequent resource builds.
You can of course L<Build specific resources> using this method too.

=back

If you use this method, you probably don't want to distribute the whole C<resources> directory including sources
and intermediate files: leave out at least the C<src/> and C<build/> subdirectories,
or use the C<publish> target of C<resources/Makefile> to publish resources;
see L<C<resources/README.txt>|https://git.zdl.org/svn-mirror/ddc-dstar/src/branch/master/resources/README.txt>
for details.
...
=head2 C<"free" !=> "non-free">

This document (and the underlying code) was originally written for the (non-free)
code-base employed by the DWDS/ZDL ecosystem.  Care has been taken to ensure that as
much functionality as reasonably possible is available via publicly accessible
repositories. In the transition to public maximal public accessibility, this
repository -- and its dependencies -- have likely diverged from whatever version(s)
 -- if any -- are still in use in that ecosystem.  Some things will be broken,  Other things
may not work as advertised.  Basic corpus compilation & runtime services have been tested and
appear to work,  Comments, bug-reports, & contributions are welcome.
...
run C<docker login> for the ZDL docker registry.  It may also be caused by a change to the
registry URL in the default L<IMAGE|/-i IMAGE> in C<dstar-gantry.sh> v0.0.4 (2021-04-21)
reflecting an upstream change in the underlying ZDL docker repository URL
(from `lex.dwds.de:443` to `docker.zdl.org`).
...
=item connection reset by peer

 Connection reset by peer.
 The connection was reset.
 The connection to the server was reset while the page was loading.

This message may appear (e.g. in your local browser) when attempting to
access a L<staging instance|/MYCORPUS staging> from your local workstation,
in particular if you are using
an L<ssh tunnel|https://www.ssh.com/ssh/tunneling/example> for
port-forwarding.
Some success has been reported using the C<-g> option to C<ssh> when
setting up the tunnel as suggested
L<here|https://serverfault.com/questions/427703/ssh-tunnel-doesnt-work>,
i.e.

 WORKSTATION $ ssh -g 8002:GANTRYHOST:8001 -N GANTRYHOST & gantry_tunnel_pid=$!


=item bind: Address already in use

 bind: Address already in use
 channel_setup_fwd_listener_tcpip: cannot listen to port: 8000
 Could not request local forwarding.

This error may appear when attempting to set up an
an L<ssh tunnel|https://www.ssh.com/ssh/tunneling/example> for
accessing a L<staging instance|/MYCORPUS staging> behind a firewall.
It indicates that the local port you have chosen for the
tunnel is already allocated to another process (which
might be a previous invocation of the same ssh-tunnel
you have forgotten to kill).  Either kill the obstructing
process or choose a different local port for your ssh tunnel;
see also L<https://askubuntu.com/questions/447820/ssh-l-error-bind-address-already-in-use>.

=item svn: E170013: Unable to connect to a repository
...
This error message is emitted if the dstar SVN repository is inaccessible, e.g.
because the virtual machine on which it resides has catastrophically failed.
It may also indicate an error with the gantry host's network connectivity.
In an emergency situtation if the dstar SVN repository is down when you need to build a new corpus
(without an existing L<C<CORPUS_ROOT>|/-C CORPUS_ROOT>),
you can use the C<dstar-buildhost> image to manually create an appropriate
L<C<CORPUS_ROOT>|/-C CORPUS_ROOT> by calling:

 $ dstar-gantry.sh -c MYCORPUS -- -- env exec dstar-nice.sh cp -naT corpus.template corpora/MYCORPUS

In order to disable implicit SVN update during dstar-gantry operations, you should
also set the C<dstar_checkout_corpus_opts="-force -dummy">
L<container environment variable|/-e VAR=VALUE> as long as the SVN repository is inaccessible.
If you need to L<publish|/publish> corpora
in the absence of a working dstar SVN repository, you should consider setting
C<PUBLISH_SVN_CHECK=no>,
C<SERVER_PUBLISH_CHECKOUT=no>,
C<WEB_PUBLISH_CHECKOUT=no>,
and C<PUBLISH_REGISTER=no>,
in which case you will need to
manually create appropriate
L<runtime checkouts|https://kaskade.dwds.de/dstar/doc/HOWTO.html#Runtime-Checkouts>
on C<RUNHOST> and/or C<WEBHOST>
(hint: you can use the gantry template for these too).
Altering these variables from their default values is B<NOT> recommended in general
and should be considered a B<last resort> for B<emergency use only>.

=item No rule to make target 'init-rml'.  Stop.
...
This error has been observed when attempting to L<run|/run> a L<staging instance|/MYCORPUS staging>
for a corpus whose L<C<CORPUS_ROOT/server>|/-C CORPUS_ROOT> directory was missing the requisite
shared dstar infrastructure code (in this case, C<CORPUS_ROOT/server/Makefile>) after failure
of the infrastructure SVN server.
To avoid this error, ensure that your L<C<CORPUS_ROOT>|/-C CORPUS_ROOT> directory is
a valid SVN working copy, and resolve any L<conflicts|/SVN conflicts> reported by C<svn status -u CORPUS_ROOT> before
attempting to L<install|/install> or L<run|/run> a staging instance for that corpus.
...
SVN conflicts can arise when attempting to update a L<C<CORPUS_ROOT>|/-C CORPUS_ROOT>
(sub)directory which contains inconsistent SVN metadata.  SVN conflicts must
be resolved manually; see L<http://svnbook.red-bean.com/> for details.

=item Warning: the RSA host key for 'HOST' differs from the key for the IP address 'IPADDR'
...
This warning message can arise due to a stale C<known_hosts> file for the
ephemeral C<dstar-build> user in the running L<C<dstar-buildhost>|/-i IMAGE>
container.  It should be non-fatal, but please report it if you encounter it
during a C<dstar-gantry.sh> operation.


=item dstar.perl: could not connect to DDC server localhost port PORT: Connection refused

This error can appear in a browser when attempting to access a
L<staging instance|/MYCORPUS staging> for which the C<ddc_daemon> process
in L<C<CORPUS_ROOT/server>|/-C CORPUS_ROOT> has not (yet) successfully started.
Check the L<C<docker logs>|https://docs.docker.com/engine/reference/commandline/logs/>
for the staging container
and/or its L<supervisord|http://supervisord.org/> GUI at C<http://CONTAINER:9001>
for more details.

=item Cannot allocate memory
...
See L<"Cannot allocate memory" in the HOWTO|/$doc/HOWTO.html#Cannot-allocate-memory>.
...
L<hub.docker.com/u/cudmuncher|https://hub.docker.com/u/cudmuncher> pre-built public docker images

=item *

L<dstar-docker|https://github.com/moocow-the-bovine/dstar-docker> github repo

=item *

L<ddc-dstar-core|https://sourceforge.net/projects/ddc-dstar-core/> public SVN repo
...
Bryan Jurish E<lt>moocow@cudmuncher.deE<gt> created the ddc-dstar corpus administration system
...
it under the terms of the European Union Public Licence.  See the
file F<LICENSE> in the root directory of the source distribution
for details.
