...
calls.  Please consult the relevant
...
If a subdirectory L<C<CORPUS_ROOT/config.local/>|$doc/README_build.html#Corpus-specific-customizations-with-config.local> exists,
it will be treated as a (sparse) local set 
of dstar-configuration overrides for L<C<CORPUS>|/-c CORPUS> and merged into a pure-local
configuration directory L<C<CORPUS_ROOT/config/>|$doc/README_build.html#DSTAR_CONFIG> by implicit calls to  C<dstar-checkout-corpus.sh>.
...
See L</Gantry Configuration Variables> for a list of known gantry
...
Just executs C<CMD...> in the embedded container.
...
=head2 Gantry Configuration Variables

The following shell variables are used internally by the C<dstar-gantry.sh> script,
and may be set or modified by L<configuration file(s)|/-f RCFILE>.  Default
values are shown where appropriate, but may differ in some situations
(e.g. running via sudo).  See the example C<dstar-gantry.rc> file in the
gantry distribution for details.
...
operations using C<dstar-gantry.sh>.  Since <dstar-gantry> is basically
...
as well, where the C<DSTAR_BUILDHOST> role is fulfilled by the embedded
...
If you want to avoid having to specify the gantry L<C<-S>|/-S CORPUS_SRC> for every C<dstar-gantry.sh>
call, you should symlink the location of the "real" sources into your
L<C<DSTAR_ROOT>|/-d DSTAR_ROOT> checkout at
L<C<DSTAR_ROOT/sources/MYCORPUS>|/-d DSTAR_ROOT>.
...
first have to check it out (gantry doesn't require or checkout C<DSTAR_ROOT/config/> by default):
...
C<CORPUS_ROOT/log/build.I<DATETIME>.log>, and some progress information
...
C<CORPUS_ROOT/log/test.I<DATETIME>.log>, and some progress information
...
<dstar-gantry.sh> to L<install|/install> and L<run|/run>
...
L<C<docker run -d>|https://docs.docker.com/engine/reference/run/#detached--d> option
to run the embedded container in the background.  When you are done with manual
testing, remember to terminate the running container with C<docker kill>.
...
C<CORPUS_ROOT/log/publish.I<DATETIME>.log>, and some progress information
will be printed to the console.
...
C<config.local/> are checked into
...
and calls L<C</dstar/corpora/MYCORPUS/build.sh -update>|$doc/HOWTO.html#build.sh> in the
in the embedded L<C<dstar-buildhost>|/-i IMAGE> container to perform the
index update.
By default, full L<build-logs|$doc/HOWTO.html#Build-Logging> will be written to
C<CORPUS_ROOT/log/update.I<DATETIME>.log>, and some progress information
will be printed to the console via 
L<C<cronit.perl -echo-preset=make-info>|$doc/HOWTO.html#cronit.perl>.
...
By default, full L<build-logs|$doc/HOWTO.html#Build-Logging> will be written to
C<CORPUS_ROOT/log/update-meta.I<DATETIME>.log>, and some progress information
will be printed to the console via 
L<C<cronit.perl -echo-preset=make-info>|$doc/HOWTO.html#cronit.perl>.
...
down the road.  Consider L<archiving the corpus build directory|/MYCORPUS build archiving>
if you need to save disk space.

=head2 log changes

When making changes to a corpus configuration in SVN, remember to
log any changes to L<DSTAR_ROOT/doc/Changes.txt|$doc/HOWTO.html#doc%2FChanges.txt>.
If you are using a
L<C<CORPUS_ROOT/config.local/>|$doc/README_build.html#Corpus-specific-customizations-with-config.local>
