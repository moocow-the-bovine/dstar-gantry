...
See the example
L<C<dstar-gantry.rc> file|http://svn.dwds.de/websvn/filedetails.php?repname=D%2A%3A+Dev-Repository&path=%2Fddc-dstar%2Fdocker%2Fgantry%2Ftrunk%2Fdstar-gantry.rc> in the C<dstar-gantry> distribution for a list of available variables.
...
See
L<C<dstar-buildhost:/dstar/init/etc_default_dstar_relay>|http://svn.dwds.de/websvn/filedetails.php?repname=D%2A%3A+Dev-Repository&path=%2Fddc-dstar%2Ftrunk%2Finit%2Fetc_default_dstar_relay> for syntax and more details.
...
This message can occur if the C<ddc_daemon> process for a
L<staging instance|/MYCORPUS staging> in L<C<CORPUS_ROOT/server>|/-C CORPUS_ROOT>
is unable to allocate sufficient L<virtual memory|https://en.wikipedia.org/wiki/Virtual_memory> address space
to accomodate its index data, possibly due to a misconfiguration of the dstar
make variable L<C<SERVER_ULIMIT_MEM>|$doc/HOWTO.html#RUNHOST-memory>.  Try raising
the value of C<SERVER_ULIMIT_MEM> or setting it to the empty string: if the error message is
no longer raised, report the adjustment and/or modify the appropriate configuration file(s)
(e.g. C<CORPUS.mak>).

Contemporary 64-bit linux architectures
can handle up to 2^48 bytes (= 256 TB) of virtual memory address space on a single machine, regardless
of the amount of physical RAM installed, so this error is unlikely to be caused by machine limitations
on the gantry host itself.
